...
int Updater::doAddJob(Job& addJob)
{
	...
	if(packageIt != addMap_.end())
	{
		...
		if(createDir(pack.env1_, 0755) < 0)
		{
			if(errno == EEXIST)
		...
		if(createDir(pack.env2_, 0755) < 0)
		{
			if(errno == EEXIST)
		...
		if(!moveFiles(updatePath_ + pack.name_ + "/", pack.env1_))
		...
		int linkSuc = createSymbolicLink(pack.symLink_, pack.env1_ + pack.exec_);
		if(linkSuc != 0)
...
int Updater::doRemoveJob(Job& rmJob)
{
	...
	if(packageIt != packageMap_.end())
	{
		...
		{
			if((deleteDir(pack.env1_) != 1) || (deleteDir(pack.env2_) != 1))
...
int Updater::writeUpdateFile(string in, string out)
{
	return rename(in.c_str(), out.c_str());
}

int Updater::getUpdateFiles(string dir, string upFile, std::vector<string>& files)
{
	...
	while ((dp = readdir(dirp)) != NULL)
	{
        ...
        if(strcmp(dp->d_name, ".") && strcmp(dp->d_name, "..") && dp->d_type == DT_DIR)
			getUpdateFiles(string(dir + dp->d_name + "/"), upFile, files);
    }
	(void)closedir(dirp);
	if(files.size() == 0)
		return log_ << "ERROR: Couldnt find Updating files at path: " << dir << endl, 0;
...
int Updater::createSymbolicLink(string name, string content)
{
	return symlink(content.c_str(), name.c_str());
}

string Updater::getCurrentAppfolder(string link)
{
...
int Updater::doUpdateJob(Job& upJob)
{
	...
	if(packageIt != packageMap_.end())
	{
		...
		if(atof(upJob.new_version_.c_str()) > atof(pack.version_.c_str()))
		{
			...
			string app_folder = getCurrentAppfolder(pack.symLink_);
			...
			if(upJob.path_.size() == 0)
				moveFiles(updatePath_ + pack.name_ + "/", copy_to_path);
			else
				moveFiles(upJob.path_, copy_to_path);
			...
			int linkSuc = createSymbolicLink(tempLink, executable);
			if(linkSuc != 0)
				return log_ << "ERROR: Couldnt create temporary symbolic link " << tempLink << " error code " << linkSuc << endl, 0;
			int writeSuc = writeUpdateFile(tempLink, pack.symLink_);
			if(writeSuc != 0)
...
int Updater::deleteDir(string dir)
{
    int ret = 1;
    FTS *ftsp = NULL;
    FTSENT *curr;
...
int	Updater::createDir(string s, mode_t mode)
{
    size_t pre=0,pos;
    std::string dir;
    int mdret = 0;
...
int	Updater::moveFiles(string move_from, string move_to)	
{
	if(!(move_to.back() == '/'))
		move_to = move_to + "/";
	std::vector<string> files;
	if(!getUpdateFiles(move_from, updateFile_, files))
		return 0;
	for(string file : files)
	{
		unsigned found = file.find_last_of("/\\");
		string filename = file.substr(found + 1, file.size() - 1);
		int suc = writeUpdateFile(file, move_to + filename);
		if(suc == 0)
